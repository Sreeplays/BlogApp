...
const CommentsForm = ({ btnLabel, formSubmitHandler, formCancelHandler }) => {
  ...
  return (
    <form onSubmit={submitHandler}>
      ...
      <div className='flex items-centre gap-x-2'>
        {formCancelHandler && (
          <button className='border border-red-500 text-red-500 py-2.5 px-6 rounded-lg mb-4 mt-2' onClick={formCancelHandler}>
            Cancel
          </button>
        )}
        <button className='bg-primary text-white py-2.5 px-6 rounded-lg mb-4 mt-2' onSubmit={formSubmitHandler}>{btnLabel}</button>
      </div>
